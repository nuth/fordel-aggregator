...
test('scrapeSource uses Klarna GraphQL API and stamps discounts', { concurrency: false }, async (t) => {
  const originalFetch = globalThis.fetch;
  t.after(() => {
    globalThis.fetch = originalFetch;
  });
            ...
            deals: [
              {
                ...
                cashbackAmount: 500,
                dealUrl: 'https://www.komplett.no/',
                isUpTo: false,
                storeKrn: 'krn:cashback:store:abc123',
              },
              {
                merchantName: 'Elkjøp',
                cashbackAmount: 300,
                dealUrl: 'https://www.elkjop.no/',
                isUpTo: true,
                storeKrn: 'krn:cashback:store:def456',
              },
            ],
          },
        ],
  ...
  globalThis.fetch = async (url, options) => {
    if (url === 'https://app-api.klarna.com/api/deals_directory_bff/public/graphql') {
      return { ok: true, json: async () => graphqlResponse };
    }
    throw new Error(`Unexpected URL: ${url}`);
  };
...
test('scrapeSource falls back to Klarna HTML when GraphQL returns empty sections', { concurrency: false }, async (t) => {
  const originalFetch = globalThis.fetch;
  t.after(() => {
    globalThis.fetch = originalFetch;
  });
              ...
              pages: [
                {
                  stores: [
                    {
                      storeKrn: 'krn:cashback:store:abc123',
                      displayName: 'Farmasiet',
                      storeDirectUrl: 'https://www.klarna.com/no/api/frontend-transition-page/gotostore/v1/home/NO/store/abc123',
                      cashbackDiscount: {
                        discountLabel: { prefix: '', body: '4', suffix: '%' },
  ...
  const pageHtml = `<html><body><script>${JSON.stringify(storeData)}</script></body></html>`;

  globalThis.fetch = async (url, options) => {
    if (url === 'https://app-api.klarna.com/api/deals_directory_bff/public/graphql') {
      return { ok: true, json: async () => ({ data: { cashbackLandingPage: { sections: [] } } }) };
    }
    if (url === 'https://www.klarna.com/no/store/?type=CASHBACK') {
  ...
  assert.deepEqual(result.discounts[0], {
    name: 'Farmasiet',
    description: '4 % cashback',
    categories: [],
    link: 'https://www.klarna.com/no/api/frontend-transition-page/gotostore/v1/home/NO/store/abc123',
    source: 'Klarna Cashback',
    sourceId: 'klarna-cashback',
    scrapedFrom: 'https://www.klarna.com/no/store/?type=CASHBACK',
    lastScraped: '2026-08-07T11:00:00.000Z',
